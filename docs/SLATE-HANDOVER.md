...
order — before touching anything. Everything below is as of commit `4cc1c84`
on branch `web-scope`, 2026-09-20.
...
into the web app as a right-hand dock called **Writer's Tools**. Two of seven
phases are built: the foundations (sidecar storage, the dock) and the first
tool, **Fill**, which finds `[SOMETHING happens]` placeholders in a script and
walks the writer through resolving one. The remaining five tools are listed
in the dock as "not built yet". The writer has just asked to be able to test
the others, so **building them, one per session in plan order, is the next
...
| Dock markup, views, handlers | `Passage/Passage.Web/Components/Pages/Editor.razor` — `<aside class="workshop">` ~389; tools view ~395; BRACKETS ~445; FILL ~478; handlers from `// ---- Writer's tools dock` ~2170 |
| Client-side pieces | `Passage/Passage.Web/wwwroot/js/passage.js` — `applyWorkshopWidth` / `initWorkshopResize` ~125, `replaceInLine` ~540, `scrollToLine(line, focus)` |
| Styles | `Passage/Passage.Web/wwwroot/css/app.css` from `/* ---- Writer's tools dock` ~954 |
| Tests | `Passage/Passage.Tests/Program.cs` — `TestBracketScanner*`, `TestSlateStore*` (the test project references `Passage.Web`) |
...
- **Decision E — burst timers:** ready (input focused *before* anything
  ...
  advances). All client-side. Not yet built anywhere; Phase 2 is the first
  place it is needed. Adam said "10 seconds for three ideas"; the vault says
  30-second bursts — durations are configurable, settle it with him.
...
1. **Phase 2 — WOAC chain + Character Flaw Brainstorm.** First use of the
   burst timer (decision E). The Consequence → next Want auto-carry *is* the
   tool.
2. **Phase 3 — A→Z→Split, Belief Split, Extend Backward.** Rung 3 of Split
   writes `#`/`##` sections into the script by ranged insert.
3. **Phase 4 — Bridge + Position.**
4. **Phase 5 — Push/Pull and Lens** (plus the optional diagnostic Belief
   Split and read-back fields).
5. **Phase 6 — Ideation burst**, full-screen overlay.
6. Phase 7, the model seam, is deliberately last and probably not wanted yet.
...
state. The Fill runner is the reference implementation.
...
- **A server may already be running.** At handover, one is: started from the
  agent's Bash tool with `nohup ~/.dotnet/dotnet run --project
  Passage/Passage.Web/Passage.Web.csproj &`, serving `http://localhost:5210`,
  because Adam's own attempts kept failing on the PATH problem. `pgrep -af
  Passage.Web` finds it; `pkill -f Passage.Web` stops it. It does **not**
  hot-reload — rebuild and restart after every change.
